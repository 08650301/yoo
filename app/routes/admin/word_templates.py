# app/routes/admin/word_templates.py
...
admin_word_templates_bp = Blueprint('admin_word_templates', __name__, url_prefix='/admin/api')
...
@admin_word_templates_bp.route('/sections/<int:section_id>/chapters', methods=['POST'])
...
@admin_word_templates_bp.route('/sections/<int:section_id>/chapters', methods=['GET'])
def get_chapter_templates(section_id):
    """获取指定分区的所有Word章节模板及其关联状态"""
...
@admin_word_templates_bp.route('/chapters/reorder', methods=['POST'])
...
@admin_word_templates_bp.route('/sheets/<int:sheet_id>/link_chapter', methods=['POST'])
def link_sheet_to_chapter(sheet_id):
    """将一个表单关联或取消关联到一个Word章节模板"""
